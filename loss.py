...
class ContourLoss(torch.nn.Module):
    ...
    def forward(self, pred, target, weight=10):
        ...
        Runs in fp32 even under autocast: the sqrt(delta_pred + 1e-8) is numerically
        fragile in bf16 (1e-8 underflows to 0 in the bf16 mantissa, sqrt(0).backward
        emits +inf). Upcasting before the gradient ops keeps gradients finite.
        '''
        pred = pred.float()
        target = target.float()
        # length term
